...
# 确保数据加载后再执行操作
if not df.empty:
    unique_debris_types = ['All types of debris'] + sorted(df['type_debris'].dropna().unique().tolist())
    selected_debris = st.selectbox('Select Type of Debris:', unique_debris_types)

    unique_pickup_addresses = ['All pickup addresses'] + sorted(df['pickup_address'].dropna().unique().tolist())
    selected_pickup_address = st.selectbox('Select Pickup Address:', unique_pickup_addresses)

    unique_receiving_addresses = ['All receiving addresses'] + sorted(df['receiving_address'].dropna().unique().tolist())
    ...
    def draw_routes(data, color):
        if not data.empty:
            ...
            color = [int(color.lstrip('#')[i:i+2], 16) for i in (0, 2, 4)] + [255]
